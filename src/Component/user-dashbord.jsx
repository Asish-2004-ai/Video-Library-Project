...
export function Userdashbord() {
  ...
  function handleChange(e) {
    ...
    if (categoryId === 'ALL') {
      Loadvideos("http://127.0.0.1:3030/get-video");
    } else {
      Loadvideos(`http://127.0.0.1:3030/get-categoryid/${categoryId}`);
    }
  }
  ...
  function handleLike(videoId) {
    const updatedVideos = videos.map(video => {
      if (video.VideoId === videoId) {
        axios.put(`http://127.0.0.1:3030/update-like/${videoId}`, { Likes: video.Likes + 1 })
          .then(() => {
            video.Likes += 1;
          });
      }
  ...
  function handleDislike(videoId) {
    const updatedVideos = videos.map(video => {
      if (video.VideoId === videoId) {
        axios.put(`http://127.0.0.1:3030/update-dislike/${videoId}`, { Dislikes: video.Dislikes + 1 })
          .then(() => {
            video.Dislikes += 1;
          });
      }
